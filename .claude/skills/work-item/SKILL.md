...
재시도 · 개선 사이클에서 ③ · ⑤에게 증거 라벨 접두(`build-r2-` 등)를 새로 지시하지 않는다. 채점 스크립트가 정확한 문자열로 찾는 라벨(계획이 정한 것)은 그대로 다시 쓰고, 채점은 같은 라벨의 마지막 로그로 한다. 새 라벨은 채점 대상이 아닌 기록에만 쓴다. (작업 D H6)
...
작업 도중 사용자가 파일 · 컨테이너 · 설정을 직접 바꿨다고 알려 주면 이 표에 한 줄씩 추가한다.

**③–⑤ 동안 메인 세션은 git이 추적하는 파일을 고치지 않는다.** 하네스 · 템플릿 변경 요청이 오면 원칙적으로 마무리 단계로 미룬다. 급해서 바로 해야 하면 고치기 전에 이 표에 시각 · 파일 · 사용자 답 원문을 먼저 적는다(작업 범위 확인 AC가 이 표와 대조한다). ④ · ⑤는 작업 창 안의 변경을 이 표와 대조해, 표에 있는 변경은 ③의 행동으로 보지 않는다.

## 마무리

1. `docs/plan.md`에서 해당 항목 체크박스를 `[x]`로 바꾼다. 원격 확인 등이 남아 아직 체크할 수 없으면 그 줄에 "로컬 완료 · 무엇 대기" 메모를 단다.
1-0. **진행 상황 갱신:** 현재 단계 시작 계획서(예: `docs/stage1-plan.md`)의 "현재 상태" 절과 작업 순서 표 "상태" 열을 고친다. 다음 세션은 여기서 현재 위치를 찾는다(CLAUDE.md 안내).
1-0-1. **README.md 최신화:** '진행 상황'(끝난 작업, `docs/stage1-plan.md` 링크), '테스트 실행 방법', 이번 작업으로 생긴 기능 개요, 위키 링크를 고친다. '개발 과정: 문제 → 해결 → 결과' 절은 지금까지 가장 중요한 문제 3개만 싣는다 — 이번 작업의 트러블슈팅 기록과 비교해 더 중요한 문제가 있으면 사례를 교체하고(수치는 위키 페이지와 대조), '지금까지의 테스트 결과' 표를 최신 수치로 고친다. README는 커밋 대상에 넣는다. `.env`를 만드는 안내 문구는 바꾸지 않는다(Claude는 `.env`를 다루지 않는다). (작업 D H14)
