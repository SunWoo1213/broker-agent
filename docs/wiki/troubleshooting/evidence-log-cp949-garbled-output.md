...
> 요약
> - 미해결 — 현재 우회 방법: 판정 토큰(`OK`·`HUMAN`·`BAD`·`summary`)과 경로는 ASCII이므로 판정 로직에는 영향이 없다. 로그의 한국어 부분이 읽기 어려울 뿐이며, 별도 조치 없이 기록만 남긴다.
> - 원인: `evidence.py`가 자식 프로세스(`.venv/Scripts/python checks/...`) stdout을 UTF-8로 디코딩하는데, 자식 Python은 stdout이 파이프일 때 Windows 한국어 로캘 인코딩(cp949)으로 쓴다. `evidence.py`는 자기 자신의 stdout만 재설정했고 자식에게 인코딩을 넘기지 않는다.
> - 재발 방지: `evidence.py`가 자식 프로세스에 `PYTHONIOENCODING=utf-8`을 넘기는 안(H13, 도구 변경 — 사용자 결정 대기). 적용 전까지 생기는 로그의 한국어 깨짐은 "기존 현상"으로만 기록한다(F-D9).

| 발생일 | 분류 | 상태 | 발견 경로 | 관련 원칙 |
|---|---|---|---|---|
| 2026-09-25 | 하네스 | 미해결 | ⑤ 테스트(사이클2) 참고, ⑥ 개선사항 계획(사이클2) 직접 확인 | — |
...
**미해결.** 판정 토큰(`OK`·`HUMAN`·`BAD`)과 경로는 전부 ASCII라 판정 결과·개수에는 영향이 없어, 이번 작업(작업 D)에서는 정책·테스트·`checks/`를 바꾸지 않고 기록만 남겼다. 기존 로그는 해시로 보호되므로 다시 쓰지 않는다.
...
- **H13** — `.claude/tools/evidence.py`: `subprocess.run([find_bash(), "-lc", command], env={**os.environ, "PYTHONIOENCODING": "utf-8"}, ...)`로 자식에게 UTF-8 표준 입출력을 강제한다. 판정 로직과 로그 형식은 바꾸지 않는다.
  - 확인 방법: 스크래치 run 폴더에서 `python .claude/tools/evidence.py <스크래치 run> enc-check ".venv/Scripts/python -c \"print('\\ud55c\\uae00')\""`를 실행해 로그 stdout에 `한글`이 그대로 있는지 본다(명령에는 한글을 직접 넣지 않고 ASCII 이스케이프로 쓴다).
- planner "하지 말 것"(제안): "채점 스크립트(`checks/`)가 한국어를 출력하면 첫 부분에서 `sys.stdout.reconfigure(encoding="utf-8")`를 한다. 판정 토큰은 ASCII로 둔다."
- **F-D9**(후속 항목): H13 적용 전까지 생기는 로그의 한국어 깨짐은 "기존 현상"으로만 기록한다.
