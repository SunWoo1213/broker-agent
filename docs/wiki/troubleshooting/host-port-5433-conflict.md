...
> 요약
> - 한 줄 해결: `docker-compose.yml`의 호스트 포트 기본값을 `POSTGRES_PORT:-5433` → `POSTGRES_PORT:-5434`로 바꿨다.
> - 원인: 포트 기본값을 "이 저장소만" 기준으로 정했고, 로컬에 이미 떠 있던 다른 프로젝트 컨테이너(`capstone2-postgres-1` 5433, `finance_postgres` 5432)와의 충돌을 계획 단계에서 확인하지 않았다.
> - 재발 방지: 계획에 "다른 프로젝트 컨테이너를 멈추거나 지우지 않는다"를 명시하고, 포트 값은 실행 전제 단계에서 LISTEN 여부를 먼저 확인하게 했다.
