...
| D15 | 에이전트 인증은 요청마다 Ed25519 서명. 공개키는 `agents` 테이블, 헤더 `X-Agent-Id` · `X-Timestamp` · `X-Nonce` · `X-Signature`, 서명 대상 = 메서드 · 경로 · 타임스탬프 · nonce · 본문 SHA-256. 타임스탬프 ±60초, nonce는 Redis `SET NX`(TTL 120초), **Redis에 닿지 않으면 거부** | MCP Streamable HTTP는 한 세션으로 여러 요청을 보내므로 세션 단위 인증이나 본문과 묶이지 않는 Bearer JWT로는 본문 바꿔치기를 알 수 없음 | 서명 · 본문 변조 · 시각 초과 · nonce 재사용 · Redis 중단 시 거부 테스트(작업 G) | 확정 (2026-09-25) |
| D16 | (1단계 한정) "누구를 대신한" 요청인지는 서명된 헤더 `X-On-Behalf-Of: <user_id>`로 받고, 게이트웨이는 그 사용자가 이 에이전트에게 위임했는지만 확인한다 | 사람 로그인(Keycloak) 전 단계에서 가장 단순한 방식. **알려진 한계:** 손상된 에이전트는 자기에게 위임한 다른 사용자를 사칭할 수 있음 → Keycloak 단계에서 사용자 토큰과 묶고, 평가 시나리오 "남의 데이터 접근" 원인 분석에 기록 | 위임 없는 사용자 대리 요청 거부 테스트(작업 H) | 확정 (1단계 한정, 2026-09-25) |
| D17 | 게이트웨이는 에이전트에게 MCP 서버(Streamable HTTP), 도구 3종에게 MCP 클라이언트. 도구 이름은 `expense.create`처럼 점 사용(SDK가 거부하면 `expense_create`로 바꾸고 기록). `tools/list`는 요청 사용자가 위임한 작업만 보여 주고, 판단은 항상 `tools/call`에서 한다. 작업마다 인자 JSON Schema를 `tool_actions`에 두고 OPA 전에 형식 오류를 거부, 금액은 원 단위 정수 | 에이전트는 게이트웨이 주소만 알고 도구에 직접 닿지 않음. 목록 숨김은 편의이고 보안 판단은 호출 시점에만 | `tools/list` 필터 · 목록에 없는 이름 호출 거부 · 스키마 위반 거부 테스트(작업 F · H · I) | 확정 (2026-09-25) |
...
| D19 | (1단계 임시) 도구는 게이트웨이만 아는 공유 비밀 `X-Broker-Secret` 헤더가 없거나 다르면 거부, 비교는 `hmac.compare_digest`. 비밀은 환경 변수로만 받고 `env.example`에는 이름만 | 1단계 목표 "브로커를 거치지 않으면 도구를 호출할 수 없다"를 먼저 성립. 공유 비밀은 작업 · 인자와 묶이지 않으므로 2단계에서 원칙 2의 임시 토큰(D4)으로 교체 | 헤더 없음 · 틀린 값 · 직접 호출 거부 테스트(작업 E) | 확정 (1단계 임시, 2026-09-25) |
| D20 | DB 접근은 SQLAlchemy asyncio + psycopg 3. Windows 로컬 · pytest는 Selector 이벤트 루프 사용 | 요청마다 위임 확인 · 한도 예약 · 감사 기록으로 DB를 여러 번 오가므로 비동기로 지연 목표(p95 20ms)를 맞춤. psycopg 비동기는 Windows 기본 Proactor 루프에서 동작하지 않음 | Windows · Linux(CI)에서 DB 통합 테스트 통과(작업 C) | 확정 (2026-09-25) |
